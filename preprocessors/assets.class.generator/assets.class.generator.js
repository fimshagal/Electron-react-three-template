...
class AssetsClassGenerator {
    static async generatePixiJsAssetsClass(options = {}) {
        const { resources, output, importPathPrefix } = options;

        const spritesImports = resources.sprites.map((sprite) => `import ${sprite.assetName}Path from "${importPathPrefix}/sprites/${sprite.fileName}";`).join('\n'); // "./../../assets/sprites/${sprite.fileName}"

        const texturesValues = resources.sprites.map((sprite) => `this._textures['${sprite.assetName}'] = await PIXI.Assets.load(\`\$\{${sprite.assetName}Path\}\`);`).join("\n");

        const textStylesValues = resources.textStyles.map((textStyle) => `this._textStyles['${textStyle.assetName}'] = new PIXI.TextStyle(${JSON.stringify(textStyle.options)});`).join("\n");
        ...
        const classContent = `
            ...
            import { IPixiAssets } from "./lib";

            const PixiAssetsSingletonLock: string = genSingletonLock("PixiAssets");
            
            export class PixiAssets implements IPixiAssets {
                private static _singleInstance: PixiAssets;
                private _textures: Dictionary<PIXI.Texture> = {};
                private _textStyles: Dictionary<PIXI.TextStyle> = {};
            
                private _isInit: boolean = false;
            
                constructor(singletonLock) {
                    if (PixiAssets._singleInstance) {
                        throw Error("Singleton constructor error");
                    }
            
                    if (singletonLock !== PixiAssetsSingletonLock) {
                        throw Error("Singleton constructor error");
                    }
                }
            
                public static getSingle(): PixiAssets  {
                    if (!PixiAssets._singleInstance) {
                        PixiAssets._singleInstance = new PixiAssets(PixiAssetsSingletonLock);
                    }
                    return PixiAssets._singleInstance;
                }
                ...
                private async initTextStyles(): Promise<void> {
                    ${textStylesValues}
                }
            
                public get textures(): Dictionary<PIXI.Texture> {
                    return { ...this._textures };
                }
            
                public get textStyles(): Dictionary {
    ...
    static async generateThreeJsAssetsClass(options = {}) {
        const { resources, output, importPathPrefix } = options;
